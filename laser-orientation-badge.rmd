...
-   This view is not a great way to look at all of this data.

-   Each column has information on the type of data that is there
...
-   In my data, the rows represent each case or participant
...
```{r}
d |> 
  select(student_id, total_points_possible, total_points_earned, percentage_earned)
```
...
```{r}
d |> 
  filter(FinalGradeCEMS > 88)
```

What happens when you change the cut-off from 70 to something else? Add
a thought (or more):

-   The number of cases in the data set was reduced.
...
```{r}
m2 <- lm(FinalGradeCEMS ~ TimeSpent + int + subject, data = d)
summary(m2)
```
